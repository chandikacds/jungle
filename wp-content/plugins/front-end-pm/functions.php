...
function fep_query_url( $action, $arg = array() ) {
	...
	$url = esc_url( fep_query_url_without_esc( $action, $arg ) );
			...
			if( $user_roles[0] == "customer"){
				//$url = get_home_url()."/dashboard/message-center".$url;
				$url = get_home_url()."/dashboard/message-center/?fepaction=messagebox";
			}else{
				//$url = get_home_url()."/my-account/message_center".$url;
				$url = get_home_url()."/dashboard/message-center/?fepaction=messagebox";
			}
