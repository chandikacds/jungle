	...
	function fep_autosuggestion_ajax() {
		...
		/*	$args = array(
			...
			*/
			...
			//custome query
			global $wpdb;
			$user_data = $wpdb->get_results("SELECT  u1.id, u1.user_nicename, m1.meta_value AS shop_name
FROM wp_users u1
...
WHERE m1.meta_value LIKE '%$searchq%' AND m2.meta_value LIKE '%$role%'");
...
FROM wp_users u1
JOIN wp_usermeta m2 ON (m2.user_id = u1.id AND m2.meta_key = 'wp_capabilities')
WHERE m2.meta_value LIKE '%$adminrole%' OR m2.meta_value LIKE '%$adminrole2%' OR m2.meta_value LIKE '%$adminrole3%'");

	foreach( $user_data as $user)
					{	
						//$display = fep_user_name($user->ID);
						$display = 'Jungle.lk - '.ucfirst($user->user_nicename);
