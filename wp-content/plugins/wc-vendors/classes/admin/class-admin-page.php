...
class WCV_Admin_Setup
{
	...
	public function commission_table_header_styles() {
	    ...
	    echo '.wp-list-table .column-totalval { width: 10%;}';
	    echo '.wp-list-table .column-totalpayble { width: 10%;}';
	     echo '.wp-list-table .column-action { width: 20%;}';
	    echo '</style>';
...
//var_dump($item);
				...
				$sqlqr = "SELECT SUM(total_shipping) AS total_shipping FROM (SELECT * FROM wp_pv_commission GROUP BY order_id, vendor_id) T1 WHERE T1.order_id=$item->order_id AND T1.vendor_id=$item->vendor_id";
                $results2 = $wpdb->get_results($sqlqr);
                $total_shipping = $results2[0]->total_shipping;
		...
		switch ( $column_name ) {
		    ...
		    case 'totalval' :
				return wc_price( $item->totalval );
			case 'totalpayble' :
				return wc_price( $item->totalval + $total_shipping - $item->total_due );
		    
		    case 'action' :
		    	if($item->status != 'paid'){
		    		$amount = $item->totalval + $total_shipping - $item->total_due;
		    		$banklist = getJungleBankList();
		    		$buttonName = '<select id="bankname_paymant'.$item->vendor_id.'_'.$item->order_id.'" name="bankname">';
		    		$buttonName .='<option value="">Select Bank</option>';
		    		foreach ($banklist as $key => $value) {
		    			$buttonName .='<option value="'.$key.'">'.$value.'</option>';
		    		}
		    		$buttonName .='</select>';
		    		$buttonName .='<input type="text" class="datepicker" name="paiddate" id="paiddate_paymant'.$item->vendor_id.'_'.$item->order_id.'" value="" placeholder="Select paid date time">';
		    		$buttonName .='<input type="text" name="reference" id="reference_paymant'.$item->vendor_id.'_'.$item->order_id.'" value="" placeholder="Enter reference">';
		  $buttonName .='<input type="hidden" name="orderid" id="orderid_paymant'.$item->vendor_id.'_'.$item->order_id.'" value="'.$item->order_id.'">';
		  $buttonName .='<input type="hidden" id="vendorid_paymant'.$item->vendor_id.'_'.$item->order_id.'" name="vendorid" value="'.$item->vendor_id.'">';
		  $buttonName .='<input type="hidden" id="totalval_paymant'.$item->vendor_id.'_'.$item->order_id.'" name="totalval" value="'.$amount.'">';
		  $buttonName .='<input type="hidden" id="total_shipping_paymant'.$item->vendor_id.'_'.$item->order_id.'" name="total_shipping" value="'.$total_shipping.'">';
		    		$buttonName .= '<button type="submit" id="paymant'.$item->vendor_id.'_'.$item->order_id.'" paymantid="paymant'.$item->vendor_id.'_'.$item->order_id.'" class="dopaymant">Paid</button>';
		    	}else{
		    		$buttonName = 'Paid';
		    	}
		     return $buttonName;
	...
	function get_columns()
	{
		$columns = array(
			...
			'total_due'  		=> __( 'Jungle Commission', 'wcvendors' ),
			...
			'totalval'       	=> __( 'Prodcut Cost', 'wcvendors' ),
			'totalpayble'       => __( 'Total Payble Amount', 'wcvendors' ),
			'action'       		=> __( 'Paymant', 'wcvendors' ),
		);
	...
	function prepare_items()
	{
		...
		$sql = "
			SELECT id, product_id, order_id, vendor_id, SUM(totalval) AS totalval, SUM(total_due) AS total_due,  SUM(qty) AS qty, total_shipping, tax, status, time total_shipping FROM {$wpdb->prefix}pv_commission
		";
