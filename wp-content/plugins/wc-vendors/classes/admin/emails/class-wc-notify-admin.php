...
class WC_Email_Notify_Admin extends WC_Email
{
	...
	function trigger( $id, $post )
	{
		...
		$email_sent = get_post_meta( $post->ID, 'wcv_addnew_product_email_sent', true ); 
		if(empty($email_sent)){
			 update_post_meta( $post->ID, 'wcv_addnew_product_email_sent', 1 );
			 $subject = $this->get_subject();

			 $heading = $this->get_headers();
		}else{
			$subject = __( 'Updated product details by seller '.$this->vendor_name, 'wcvendors' );
			//$heading = __( 'Update product submitted: '.$this->product_name, 'wcvendors' );
			$this->heading 			= __( 'Update product submitted: '.$this->product_name, 'wcvendors' );
			$heading = $this->get_headers();
		}
		...
		if(!$this->emailround){
			$this->send( $this->get_recipient(), $subject, $this->get_content(), $heading, $this->get_attachments() ); 
		}
		
		
		$this->emailround = $this->emailround + 1;
	}
