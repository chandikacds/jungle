...
$res = preg_replace("/[^0-9]/", "", $keyword );
$postid = (int)$res;

$search = $keyword;
$qrystr = " OR (";

$brands = getTermData('woo_brands', $search );

if($brands){
    $brandId = array();
    foreach ($brands as $keyWoo => $valueWoo) {
       

        $brandId[] = "pm.meta_key LIKE '%wcv_custom_product_brand%' AND pm.meta_value = '".$valueWoo->ID."'";

    }
}

if(isset($brandId)){
    $brandstr = implode(" OR ", $brandId);
}
...
$models = getTermData('woo_model', $search );

if($models){
    $modelId = array();
    foreach ($models as $keyWoomodel => $valueWoomodel) {
       

        $modelId[] = "pm.meta_key LIKE '%wcv_custom_product_model_number%' AND pm.meta_value = '".$valueWoomodel->ID."'";

    }
}

if(isset($modelId)){
    $modelstr = implode(" OR ", $modelId);
}
...
$mpns = getTermData('woo_mpn', $search );

if($mpns){
    $mpnId = array();
    foreach ($mpns as $keympns => $valuempns) {
       

        $mpnId[] = "pm.meta_key LIKE '%wcv_custom_product_manufacturer_part_number%' AND pm.meta_value = '".$valuempns->ID."'";

    }
}

if(isset($mpnId)){
    $mpnstr = implode(" OR ", $mpnId);
}
...
$products = $wpdb->get_results( "SELECT p.post_title, p.ID 
    ...
    AND p.post_parent = '0' GROUP BY p.ID LIMIT 10", OBJECT ); 
			...
			if ( !empty( $products ) ) {
				...
				foreach ( $products as $post ) {
					...
					$productIdforjsku = getMenProduct($post->ID);
                   // $product = displayLogic($mainProid,$productIdforjsku, $product);
