...
*/ 
		...
		if ( empty( $email )) {
			return new WP_Error( 'registration-error-invalid-email', __( 'Please provide a valid Email/Mobile.', 'woocommerce' ) );
		}else{
			if ( ! is_email( $email ) ) {
				if ( username_exists( $email ) ) {
						return new WP_Error( 'registration-error-email-exists', apply_filters( 'woocommerce_registration_error_email_exists', __( 'An account is already registered with your Email/Mobile. Please log in.', 'woocommerce' ), $email ) );
					}
			}
		}
		...
		if ( 'no' === get_option( 'woocommerce_registration_generate_username' ) || ! empty( $username ) ) {
			...
			$username = sanitize_user( current( explode( '@', $email ) ), false );
		...
		$displayname = $_REQUEST["billing_first_name"];

		if(!empty($_REQUEST["billing_last_name"])){
         	$displayname = $displayname." ".$_REQUEST["billing_last_name"];
		}
		...
		*/
		...
		$customer_id = wp_insert_user_jungl( $new_customer_data );
		...
		if ( is_wp_error( $customer_id ) ) {
			...
			if(strpos($email, '+') !== false){
				update_user_meta( $customer_id, 'billing_phone', $email  );
				$keyid = mt_rand(100000, 999999);
				// phone activation

				global $wpdb;    
		       
		      			/* $output =  $wpdb->update('wp_users', array('user_activation_key'=>'$keyid', 'user_status '=>2), array('ID'=>$customer_id)); */

		      			 $wpdb->query( $wpdb->prepare( "UPDATE wp_users SET user_activation_key = '%s', user_status = '%d'  WHERE ID = '%d'", $keyid, 2, $customer_id) );

	   $message ='Your%20Activation%20PIN%20is%20'.$keyid.'.%20www.jungle.lk%200112%20100%20100';
       $phone_num = $email;
       sendSMS($phone_num,$message,1);
