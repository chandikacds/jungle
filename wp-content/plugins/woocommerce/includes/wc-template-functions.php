...
if ( ! function_exists( 'woocommerce_form_field' ) ) {
	...
	function woocommerce_form_field( $key, $args, $value = null ) {
		...
		if (strpos($label_id, 'shipping') !== false) {
			    $field_container = '<p class="shipping-row form-row %1$s %2$s" id="%2$s" data-priority="' . esc_attr( $sort ) . '">%3$s</p>';

			}
			if (strpos($label_id, 'billing') !== false) {
				if($label_id == 'billing_billing_postcode'){
						$field_container = '<p class="billing-row form-row %1$s %2$s" id="%2$s" data-priority="120">%3$s</p>';
				}else{
					$field_container = '<p class="billing-row form-row %1$s %2$s" id="%2$s" data-priority="' . esc_attr( $sort ) . '">%3$s</p>';
				}
