...
class WC_Form_Handler {
	...
	public static function redirect_reset_password_link() {
		if ( is_account_page() && ! empty( $_GET['key'] ) && ! empty( $_GET['login'] ) ) {
			...
			if (strpos($_GET['login'], 'plus94') !== false) {
			$value = sprintf( '%s:%s', $_GET['login'], wp_unslash( $_GET['key'] ) );
			$value = str_replace('plus', '+', trim($value));
	...
	public static function save_account_details() {
		...
		if ( wc_notice_count( 'error' ) === 0 ) {
			...
			if ( $customer ) {
				...
				if ( $current_billing_phone !== $_REQUEST["billing_phone"] ) {
					$customer->set_billing_phone( $_REQUEST["billing_phone"] );
				}
	...
	public static function process_lost_password() {
		if ( isset( $_POST['wc_reset_password'] ) && isset( $_POST['user_login'] ) && isset( $_POST['_wpnonce'] ) && wp_verify_nonce( $_POST['_wpnonce'], 'lost_password' ) ) {
				...
				if (strpos($_POST['user_login'], '+94') !== false) {
				    $mobilereset = 1;
				}else{
			...
			if ( isset($mobilereset) ) {

				global $wpdb;
				$userlogin = $_POST['user_login'];

				$check_id = $wpdb->get_results("SELECT ID FROM `wp_users` where user_login = '$userlogin' ");

				if($check_id){

				    $customer_id = $check_id[0]->ID;

				    $keyid = mt_rand(1000, 9999);

				     $wpdb->query( $wpdb->prepare( "UPDATE wp_users SET user_activation_key = '%s'  WHERE ID = '%d'", $keyid, $customer_id) );
				...
				wp_redirect( add_query_arg( 'reset-link-sent', $userlogin, wc_get_account_endpoint_url( 'lost-password' ) ) );
				exit;
			}
