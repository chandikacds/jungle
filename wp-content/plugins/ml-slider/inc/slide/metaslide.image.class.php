...
class MetaImageSlide extends MetaSlide {
    ...
    public function get_admin_tabs() {
       ...
       // $url = esc_attr(get_post_meta($slide_id, 'ml-slider_url', true));
        ...
        $url = get_home_url()."/offers?id=".$this->slide->ID;
        ...
        $products_tab = '<div style="width: 50%;float: left;
padding: 10px 5px;">
        <input type="text" name="searchItem" id="searchItem" slider="'.$slide_id.'" pid="result'.$slide_id.'" class="searchItem" placeholder="Start to search prodcut here..."  style="width: 100%">
        <div style="width: 100%" id="result'.$slide_id.'"></div>
        </div><div style="width: 50%;float: left;
padding: 5px;" id="saveditem'.$slide_id.'">'.showSliderProductList($slide_id).'
        ...
        $tabs = array(
            ...
            'products' => array(
                'title' => __("Products", "ml-slider"),
                'content' => $products_tab
            )
        );
