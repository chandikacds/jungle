...
class WCVendors_Pro_Product_Controller {
	...
	public function process_submit() { 
		...
		$productType = checkProductIsSub($_POST[ 'post_id' ]);

		if(giveCurrentUerSellerDashbaordAccess()){
			$productType = 1;
				}
		...
		if ( isset( $_POST[ 'post_id' ] ) && is_numeric( $_POST[ 'post_id' ] ) ) { 
			...
			if ( $post_id ) {
				...
				if ( isset( $_POST[ 'draft_button' ] ) ) {
					...
					$post_name = sanitize_title( $post_title ).'_'.$_POST[ 'post_id' ];
	  
    			 $wpdb->query( $wpdb->prepare( "UPDATE wp_posts SET post_status = '%s', post_name = '%s'  WHERE ID = '%d'", 'publish',  $post_name, $_POST[ 'post_id' ]) );
    			 ...
    			 //Set product hidden: 
					...
					if(!giveCurrentUerSellerDashbaordAccess()){
					wp_set_object_terms( $_POST[ 'post_id' ], $terms, 'product_visibility' );
				}
						...
						$text[ 'notice' ] = sprintf( apply_filters( 'wcv_product_review_msg', __( 'Product submitted for review. <a href="">Preview product.</a>', 'wcvendors-pro' ) ), $view );
					}
				}
			...
			if ( $post_id ) { 
				if ( isset( $_POST[ 'draft_button' ] ) ) { 
					...
					if ( $can_submit_live ) { 
						...
						$text[ 'notice' ] = sprintf( apply_filters( 'wcv_product_review_msg', __( 'Product submitted for review. <a href="'.$post_id.'">Preview product.</a>', 'wcvendors-pro' ) ), $view );
					}
				}
			} else { 
	...
	public function process_duplicate( ) { 
		...
		if ( isset( $wp->query_vars[ 'object' ] ) ) {
			...
			if ( $object == 'product' && $action == 'duplicate' && is_numeric( $id ) ) { 
				...
				add_post_meta( $new_product_id, 'jungl_prodcut_type', 2 );

				//other seller product dublicate
				if(isset($_REQUEST["ospd"]) && $_REQUEST["ospd"] == 1 ){

					// var_dump($authorID);
					// var_dump(get_current_user_id());
					//woo_same_product_sub_prodcuts
						if(get_current_user_id() != $authorID){
						

						//this add for remove subprodcut prom cutom search
						

						}else{
					
							$parent = $id;
							$checkid = get_post_meta( $id, 'my_main_parent', true );
							if(!empty($checkid)){
								add_post_meta( $new_product_id, 'my_main_parent', $checkid );
								$parent = $checkid;
							}else{
								add_post_meta( $new_product_id, 'my_main_parent', $id );
							}
					...
					update_post_meta( $new_product_id, 'jungl_visibility', 'hidden' );
					...
					// Update the post into the database
					  wp_update_post( $my_post );
	  				  update_post_meta( $new_product_id, 'wcv_custom_product_conditions', 1 );
	  				  update_post_meta( $new_product_id, '_stock', 0 );
	  				  update_post_meta( $new_product_id, '_sku', '' );
	  				  update_post_meta( $new_product_id, 'productServicesMeta', '' );
	  				  wp_update_post( array( 'ID' => $new_product_id, 'post_status' => 'publish' ) ); 
  	
	  				  

					wc_add_notice( apply_filters( 'wcv_product_duplicated_msg', __( 'Product Created.', 'wcvendors-pro' ) ) ); 

				     wp_safe_redirect( WCVendors_Pro_Dashboard::get_dashboard_page_url( 'product/edit/' . $new_product_id.'/?ospd=1' ) ); 
	...
	private function duplicate_product( $object_id ){ 
		...
		$newID = $new_product->get_id();
	
    global $wpdb;
     $query = "INSERT INTO wp_pordcutmeta (post_id,meta_key,meta_value) VALUE($object_id,'woo_same_product_sub_prodcuts','$newID')";
     $wpdb->query($query);


   
        //update_post_meta( 7357, 'woo_same_product_sub_prodcuts', 7369 );

        //var_dump($object_id);
	//var_dump($new_product->get_id());exit;
	...
	public function save_product( $post_id = 0 ) {
		...
		$productType = checkProductIsSub($post_id);
		...
		if ( !empty( $wcv_custom_tax ) ) { 
			...
			foreach ( $wcv_custom_tax as $taxonomy => $data) {
				...
				wp_set_object_terms( $post_id, $tax_data, $taxonomy ); 
	...
	public function table_columns( $columns ) {
		...
		$columns = array( 
					...
					'Services'  => __( 'Services', 								'wcvendors-pro' ),  
	...
	public function table_rows( $rows, $result_object ) {
		...
		foreach ( $rows as $row ) {
			...
			$categories_label 		= apply_filters( 'wcv_product_row_category_label', __( 'Categories:', 'wcvendors-pro' ), $product, $product->get_id() ); 
			$tags_label 			= apply_filters( 'wcv_product_row_tags_label', __( 'Tags:', 'wcvendors-pro'), $product, $product->get_id() ); 
			$stock_status 			= "";
			$stock_status_label  = "";
			/*
			...
			*/
			...
			//pass Product ID and output Condition Value
			...
			$product_categories		= ( version_compare( WC_VERSION, '2.7', '<' ) ) ? $product->get_categories() : wc_get_product_category_list( $product->get_id() ); 
			$product_tags 			= ( version_compare( WC_VERSION, '2.7', '<' ) ) ? $product->get_tags() : wc_get_product_tag_list( $product->get_id() ); 
//var_dump($product);
			...
			foreach ($outputserviceID as $key => $value) {
				if (!empty($value)) {
					...
					$Servicesar[]="Services Not Available";
				}
	...
	public function table_actions() {
		...
		$can_submit_products  	= WC_Vendors::$pv_options->get_option( 'can_submit_products' );
	...
	*/
	public static function get_product_edit_link( $product_id = null, $default = false ){ 
		...
		$default_template 	= WC_Vendors::$pv_options->get_option( 'product_form_template' ); 
	...
	*/
	public static function get_default_product_template(){ 
		...
		$template 		= WC_Vendors::$pv_options->get_option( 'product_form_template' );  
