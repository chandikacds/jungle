...
class WCVendors_Pro_Dashboard {
	...
	public static function check_object_permission( $object, $post_id ) { 
		...
		if(giveCurrentUerSellerDashbaordAccess()){
			$can_edit_live = true;
			$current_user_id = WCV_Vendors::get_vendor_from_product( $post_id );
		}
		...
		switch ( $object ) {
			// Product permissions 
			case 'product':
				return ( $can_edit_live && WCV_Vendors::get_vendor_from_product( $post_id ) == $current_user_id ) ? true : false; 
				break;
			case 'shop_coupon':
	...
	*/
	public function create_nav( ) { 
		...
		if ( !$viewstore_disabled ) { 
			$shop_name =  get_user_meta( get_current_user_id(), 'pv_shop_name', true );

			$shopName = WCVendors_Pro_Vendor_Controller::get_vendor_store_url( get_current_user_id() );
			$shopName = str_replace("vendors/","myshop/?shopname=",rtrim($shopName,'/')).'&id='.my_simple_crypt( get_current_user_id());
			$store_url 		= apply_filters( 'wcv_dashboard_view_store_url', array( 'label' => __( 'View Store', 'wcvendors-pro' ),'slug' => $shopName ) ); 
