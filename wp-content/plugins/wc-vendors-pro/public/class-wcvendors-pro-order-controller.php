	...
	public function table_rows( ) {
		...
		if ( !empty( $all_orders ) ) { 
			...
			foreach ( $all_orders as $_order ) { 
				...
				$order 			= $_order->order; 
				$order_idforcheck 		= ( version_compare( WC_VERSION, '2.7', '<' ) ) ? $order->id : $order->get_id();
				// var_dump($order_idforcheck);
				
				$output= get_post_meta( $order_idforcheck, 'admin_aprovel_for_seller', true );
				...
				if ( !empty( $_order->order_items ) ){ 
					foreach ( $_order->order_items as $item ) {
						...
						//To get deteils to Products Colounm
						$sellerNote = get_post_meta($product_id,'_wcv_custom_product_seller_note');
						$mpn = get_post_meta($product_id,'wcv_custom_product_manufacturer_part_number');
						$jsku = generateJsku($product_id);
						$sku = get_post_meta($product_id,'_sku');

						if (!empty($sellerNote[0])) { $sellerNoteVal = $sellerNote[0];}
						if (!empty($mpn[0])) { $mpnName = get_the_title( $mpn[0] );} 
						if (!empty($sku[0])) { $skuVal =  $sku[0];}
						$conditionsList = productConditions();
						$conditionsid = get_post_meta( $product_id, 'wcv_custom_product_conditions', true );
						...
						// $products_html .= '<strong>'. $item['qty'] . ' x ' . $item['name'] . '</strong><br />'. $sellerNote[0] . '<br />' . $mpnName. '<br />' . $conditions  ; 
						$products_html .= '<strong>'. $item['name'] . '</strong><br>';

						if(!empty($sellerNoteVal)){
							$products_html .= '<b> Seller Note  : </b> '. $sellerNoteVal . '<br />';
						}
						if(!empty($mpnName)){
							$products_html .= '<b> Manufacturer part number&nbsp&nbsp&nbsp: </b>' . $mpnName. '<br />';
						}
						if(!empty($conditions)){
							$products_html .= '<b> Condition  : </b>' . $conditions.'<br />';
						}
						if(!empty($jsku)){
							$products_html .= '<b> JSKU  : </b>'.$jsku. '<br /> ';
						}
								
						if(!empty($skuVal)){
							$products_html .= '<b> SKU  : </b>'.$skuVal.' ';
						}
						$list_branch = action_display_branches( $order->id, $product_id,1 );

						if(isset($list_branch)){
							$products_html .= '</br><b> Branch  : </b>'.$list_branch.' '; 
						}
						$products_html .='<br><br>';
						$eachqty .= $item['qty'].'<br>';
				...
				if ( __( 'No', 'wcvendors-pro' ) == $shipped ) {
					...
					$row_actions['mark_shipped'] = array( 
						...
						'class'		=> 'markShipped wcv-button'	 
					); 
				}else{
					$row_actions['unmark_shipped'] = array( 
						'label' 	=> __( 'Unmark Shipped', 'wcvendors-pro' ), 
						'url' 		=> '?wcv_mark_shipped='.$order->get_order_number(),
						'class'		=> 'unmarkShipped wcv-button'
				...
				foreach ( $_order->order_items as $item_id => $item ) {
					if ( isset( $item['line_total'] ) ) {
						if ( isset( $item['line_subtotal'] ) && $item['line_subtotal'] != $item['line_total'] ) {
							$total_text 		.= '<del>' . wc_price( $order->get_item_subtotal( $item, false, true ), array( 'currency' => $order_currency ) ) . '</del> ';
						}
						$total_text 		.= wc_price( $order->get_item_total( $item, false, true ), array( 'currency' => $order_currency ) );
					}
					$total_text 		.= '<br>';
				}
				...
				/*$total_text 		= '<span class="wcv-tooltip" data-tip-text="'. sprintf( '%s %s %s %s %s %s', __( 'Product: ' , 'wcvendors-pro'), $commission_due, __( 'Shipping: ' , 'wcvendors-pro'), $shipping_due, __( 'Tax: ' , 'wcvendors-pro'), $tax_due ).'">'.wc_price( $total_new ).'</span>'; 
*/
				...
				$orderStatus = '<br><select class="inline select" id="orderStatusList'.$order->get_order_number().'">
              						<option value="" >  Change Order Status </option>
				  					<option value="wc-awaiting-collect" >Awaiting Collection</option>
				  					<option value="wc-awaitservice-deli" >Awaiting Service Delivery</option>
				  					<option value="wc-collected" >Collected </option>
				  					<option value="wc-goods-delivered" >Goods Delivered </option>
				  					<option value="wc-processing" >Processing  </option>
				  					<option value="wc-service-supplied" >Service Supplied </option>
				  					<option value="wc-shipped" >Shipped </option>
			      				</select>
			  				    <button type="button" class="btn btn-primary btnOrderStatus inline" orderId="'.$order->get_order_number().'">✓</button> ';
				...
				$new_row->quantity 		= $eachqty;
				$new_row->total 		= $total_text;
				//$new_row->total 		= $total_new;
				$new_row->status 		= $shipped;
				$new_row->order_date    = '';
				$metavalue = get_post_meta( $order->get_order_number(), 'wc_shipping_by_seller' );
				if(!in_array(get_current_user_id(), $metavalue)){
					$new_row->order_date    .= '<p><stong>Your order will be shipped by Jungle.lk</strong></p>';
				}
				
				$new_row->order_date	.= date_i18n( wc_date_format(), strtotime( $order_date ) ) . '<br /><strong> Current Status : ' . ucfirst( wc_get_order_status_name( $order->get_status() ) ) . '</strong> <br> <strong>'.$orderStatus.'</strong>'; 
				// $new_row->order_date	= date_i18n( wc_date_format(), strtotime( $order_date ) ) . '<br /><strong >' . $orderStatus . '</strong>'; 
				// $new_row->order_date	= date_i18n( wc_date_format(), strtotime( $order_date ) ) .$orderStatus ; 
